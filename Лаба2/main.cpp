...
public:
	...
	void bringing_to_0_360(){
		...
		q=get_minute()/60;
		set_degree(get_degree()+q);                           //ïîëó÷àåì ÷èñòûå ãðàäóñû
		c=get_degree()/360;
		b.set_minute(get_minute()-q*60);                      //ïîëó÷àåì ÷èñòûå ìèíóòû
		if(get_degree()>=0){
			b.set_degree(get_degree()-c*360);
		} else {
			b.set_degree(get_degree()+(c*(-1)+1)*360);        //ò.ê. ÷èñëà îòðèöàòåëüíûå â äàííîì ñëó÷àå, äåëàåì åù¸ îäèí ëèøíèé êðóã
			if(b.get_minute()!=0){
...
Angle operator+(Angle a, Angle b){
	...
	double c=a.get_degree()*60+a.get_minute()+b.get_degree()*60+b.get_minute();  // ïåðåâîäèì âñ¸ â ìèíóòû
	int C=c/60;                                                                  // ìèíóòû â ãðàäóñû
	double m=c-C*60;                                                             // îñòàòîê ìèíóò
	Angle g((c-m)/60,m);
	if(g.get_degree()<0){
...
// ìèíóòû íå ìîãóò áûòü îòðèöàòåëüíûìè è çàâèñÿò îò çíàêà ãðàäóñîâ
